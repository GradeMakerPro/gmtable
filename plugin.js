...
(function (domGlobals) {
  ...
  var updateAdvancedProps = function (modifiers, data) {
    ...
    var hasHiddenBorder =
      ...
      data.borderstyle === 'hidden' ||
      data.borderwidth === '0';
    if (hasHiddenBorder) {
      modifiers.setAttrib('class', 'has-hidden-border');
    } else {
      modifiers.setAttrib('class', '');
    }
  };
  ...
  var styleTDTH = function (dom, elm, name, value, hasHiddenBorder) {
    if (elm.tagName === 'TD' || elm.tagName === 'TH') {
      ...
      if (hasHiddenBorder) {
        elm.className = 'has-hidden-border';
      } else {
        elm.className = elm.className.replace('has-hidden-border', '');
      }
    } else {
      if (elm.children) {
        for (var i = 0; i < elm.children.length; i++) {
          styleTDTH(dom, elm.children[i], name, value, hasHiddenBorder);
        }
      }
    }
  };
  var applyDataToElement = function (editor, tableElm, data) {
    ...
    var hasHiddenBorder =
      data.borderstyle === 'none' ||
      data.borderstyle === 'hidden' ||
      data.border.indexOf('0') === 0;

    attrs.class = data.class || '';
    if (hasHiddenBorder) {
      attrs.class += ' has-hidden-border';
    } else {
      attrs.class = attrs.class.replace('has-hidden-border', '');
    }
  ...
  var addButtons = function (editor, selectionTargets) {
    editor.ui.registry.addMenuButton('table', {
      ...
      fetch: function (callback) {
        const hierarchy = editor._nodeChangeDispatcher.lastPath;
        if (
          ['TD', 'TR', 'TBODY'].includes(editor.gmSelectedNode) ||
          hierarchy[0].nodeName === 'TABLE'
        ) {
          return callback(
  ...
  function Plugin(editor) {
    ...
    editor.on('click', e => {
      editor.gmSelectedNode = e.target.nodeName;
    });
